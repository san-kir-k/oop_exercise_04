...
struct Octagon {
    ...
    Octagon();
    Octagon(vertex center, vertex v);
    Octagon(std::istream& is);
    ~Octagon() = default;
};

template <class T>
Octagon<T>::Octagon(): center(), v(), valid(false) {}

template <class T>
Octagon<T>::Octagon(vertex center, vertex v): center(center), v(v) {
    valid = true;
    validCheck(*this);
}

template <class T>
Octagon<T>:: Octagon(std::istream& in) {
